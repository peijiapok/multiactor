...
df=pd.read_csv(OUT/"instrument_counterfactual_20260717.csv")   # 20 seeds + bootstrap CIs
piv=df.pivot(index="dev_target",columns="mode",values="all_pass")
lo=df.pivot(index="dev_target",columns="mode",values="all_pass_lo")
hi=df.pivot(index="dev_target",columns="mode",values="all_pass_hi")
x=piv.index.values*100
def ebar(c): m=piv[c].values; return [m-lo[c].values, hi[c].values-m]

fig,ax=plt.subplots(figsize=(6.0,4.0))
ax.errorbar(x,piv["self_service"].values,yerr=ebar("self_service"),fmt='-o',color='#c0392b',lw=2.4,ms=6,capsize=3,label='self-service override (reserve/price churn)')
ax.errorbar(x,piv["benign"].values,yerr=ebar("benign"),fmt='-s',color='#27ae60',lw=2.4,ms=6,capsize=3,label='override motive removed (abstain)')
ax.axhline(0,color='gray',lw=0.6)
ax.annotate('motive removed,\nsame rate', xy=(2.5,0.60), xytext=(3.8,0.64), fontsize=8,
...
ax.set_title('Targeting the diagnosed override motive recovers feasibility at fixed\n'
             'deviation rate (35\\% capacity, 20 seeds, 95\\% CIs); full recovery also needs lower deviation',fontsize=9)
