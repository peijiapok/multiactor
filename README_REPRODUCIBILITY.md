...
- `code/` — analysis scripts (Python) and derived result data (CSV/JSON):
  ...
  - `run_apartment_vs_workplace_*.py` — cross-ecology generalization: same 14-gate framework on workplace (metro_caltech) vs Korean apartment/MUD (metro_korea) scenarios across the severity sweep; shows the binding actor is context-dependent and the apartment grid benefit is compliance-contingent (`fig_apartment_vs_workplace`).
  - `run_information_integrity_*.py` — information-integrity (second compliance axis): drivers misreport urgency to game the scheduler while gates score true service; feasibility collapses by ~5% misreporting, same cliff as action non-compliance (`fig_information_integrity`). Includes a faithfulness check (replicated scheduler reproduces the truthful baseline at ~0 misreporters).
