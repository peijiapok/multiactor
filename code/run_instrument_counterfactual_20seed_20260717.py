...
def main():
    ...
    for mode in ("self_service","benign"):
        for d in DEV_GRID:
            ap_arr,dr=run_cell(mav2,d,mode)
            m,lo,hi=boot_ci(ap_arr)
            recs.append(dict(mode=mode,dev_target=d,dev_realized=round(dr,4),
                             all_pass=round(m,4),all_pass_lo=round(lo,4),all_pass_hi=round(hi,4),n_rows=len(ap_arr)))
            print(f"{mode:12s} d={d:.3f} (realized {dr:.4f}) -> all_pass={m:.4f} [{lo:.4f},{hi:.4f}] n={len(ap_arr)}",flush=True)
